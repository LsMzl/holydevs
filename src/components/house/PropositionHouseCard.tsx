...
import { PropositionHouseTypes } from "@/types/house/houseDetails";
import { MapPin, Ticket } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import React from "react";

export const PropositionHouseCard = ({ house }: PropositionHouseTypes) => {
         ...
         <Link href={`annonce/${house.id}`}>
            ...
            <div className="absolute p-4 text-white z-20 bottom-0 left-0 space-y-1">
               ...
               <div className="flex items-center gap-1">
                  <MapPin size={15} />
                  <p className="text-xs">{house.city}</p>
               </div>
               <div className="flex items-center gap-1">
                  <Ticket size={15} />
